...
export default class ListBookings {
  ...
  async execute(
    ...
    if (!(tenant as any)._isActive) {
      throw new AppError('Tenant inativo', 403);
    }
    // Lista agendamentos do tenant
    if (typeof (this.bookingRepository as any).findByTenantId === 'function') {
      let bookings = await (this.bookingRepository as any).findByTenantId(tenantId);
      // Filtros opcionais
      if (filters) {
