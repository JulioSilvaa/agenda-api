...
describe("Unit test CreateTenant UseCase", () => {
  ...
  describe("Email Uniqueness Validation", () => {
    ...
    test("should reject duplicate email even with different data", async () => {
      ...
      await expect(() =>
        createTenant.execute(duplicateEmailTenant)
      ).rejects.toThrow("Já existe um tenant com este email");
    });
  ...
  describe("Entity Validation Errors", () => {
    test("should throw error for invalid name", async () => {
      ...
      await expect(() =>
        createTenant.execute(invalidNameTenant)
      ).rejects.toThrow("Nome do tenant deve ter pelo menos 3 caracteres");
    });
    ...
    test("should throw error for invalid email format", async () => {
      ...
      await expect(() =>
        createTenant.execute(invalidEmailTenant)
    ...
    test("should throw error for invalid slug", async () => {
      ...
      await expect(() =>
        createTenant.execute(invalidSlugTenant)
      ).rejects.toThrow(
    ...
    test("should throw error for invalid phone", async () => {
      ...
      await expect(() =>
        createTenant.execute(invalidPhoneTenant)
    ...
    test("should throw error for multiple invalid fields", async () => {
      ...
      await expect(() =>
        createTenant.execute(multipleErrorsTenant)
      ).rejects.toThrow();
    });
  });
