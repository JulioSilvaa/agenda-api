...
// Garantir uso de banco em memória para cada execução
...
describe('Integration test CustomerRepositorySqlite', () => {
  ...
  describe('FindByTenantId', () => {
    test('should isolate customers per tenant', async () => {
      ...
      expect(list1.every(c => c.tenantId === tenantId)).toBe(true);
      expect(list2.every(c => c.tenantId === tenant2Id)).toBe(true);
    });
